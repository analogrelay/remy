...
pub fn exec<M>(cpu: &mut Mos6502, mem: &M, op: Operand) -> Result<(), exec::Error> where M: Memory {
    let n = try!(op.get_u8(cpu, mem)) as isize;
...
mod test {
    ...
    pub fn sbc_sets_flags_when_overflow() {
        let mut cpu = init_cpu();
        sbc::exec(&mut cpu, &mut mem::Empty, Operand::Immediate(172)).unwrap();
    ...
    pub fn sbc_subtracts_bcd_when_bcd_flag_set() {
        ...
        sbc::exec(&mut cpu, &mut mem::Empty, Operand::Immediate(0x24)).unwrap();
    ...
    pub fn sbc_sets_overflow_when_bcd_subtraction_underflows() {
        ...
        sbc::exec(&mut cpu, &mut mem::Empty, Operand::Immediate(0x01)).unwrap();
