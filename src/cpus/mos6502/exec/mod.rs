...
pub fn dispatch<M>(inst: Instruction, cpu: &mut Mos6502, mem: &mut M) -> Result where M: mem::Memory {
    match inst {
        ...
        Instruction::ISC(op) => { try!(inc::mem(cpu, mem, op)); sbc::exec(cpu, mem, op) },
        Instruction::JMP(op) => jmp::exec(cpu, mem, op),
        Instruction::JSR(addr) => jsr::exec(cpu, mem, addr),
        ...
        Instruction::RLA(op) => { try!(rotate::left(cpu, mem, op)); and::exec(cpu, mem, op, true) },
        ...
        Instruction::SBC(op) => sbc::exec(cpu, mem, op),
