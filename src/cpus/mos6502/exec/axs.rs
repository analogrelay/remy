...
pub fn exec<M>(cpu: &mut Mos6502<M>, op: Operand) -> exec::Result where M: Memory {
    let val = (cpu.registers.a & cpu.registers.x) - try!(op.get_u8(cpu));
...
mod test {
    ...
    pub fn axs_does_its_crazy_business() {
        ...
        axs::exec(&mut cpu, Operand::Immediate(0x01)).unwrap();
    ...
    pub fn axs_sets_carry_and_sign_if_result_bit_7_is_set() {
        ...
        axs::exec(&mut cpu, Operand::Immediate(0x00)).unwrap();
